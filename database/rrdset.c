...
int rrdset_set_name(RRDSET *st, const char *name) {
    ...
    if(st->name) {
        ...
        st->name = strdupz(b); //config_set_default(st->config_section, "name", b);
        st->hash_name = simple_hash(st->name);
        rrdsetvar_rename_all(st);
    }
    else {
        st->name = strdupz(b); //config_get(st->config_section, "name", b);
        st->hash_name = simple_hash(st->name);
    }
...
//    enabled = 1;
    ...
    st->chart_type = chart_type; //rrdset_type_id(config_get(st->config_section, "chart type", rrdset_type_name(chart_type)));
    st->type       = strdupz(type); //config_get(st->config_section, "type", type);

    st->state = mallocz(sizeof(*st->state));
    st->family     = family?strdupz(family):strdup(st->type); //config_get(st->config_section, "family", family?family:st->type);
    st->state->old_family = strdupz(st->family);
    json_fix_string(st->family);

    st->units      = units?strdupz(units):strdupz(""); //config_get(st->config_section, "units", units?units:"");
    json_fix_string(st->units);

    st->context    = context?strdupz(context):strdupz(st->id); //config_get(st->config_section, "context", context?context:st->id);
    st->state->old_context = strdupz(st->context);
    json_fix_string(st->context);
    st->hash_context = simple_hash(st->context);

    st->priority = priority; //config_get_number(st->config_section, "priority", priority);
    //if(enabled)
    ...
    st->title = strdupz(title); //config_get(st->config_section, "title", title);
